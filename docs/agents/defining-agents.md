...
`resolve` runs with current authority on every run, including resume. `setup` runs only on initial
creation. Repository and initial revision must remain unchanged to resume existing files.

Omitting `network` permits only the Sixb API and source origin. Explicit policies are never widened:
allow the API and initial Git clone destination, plus any package registries needed by `setup`.
An incompatible policy fails before provisioning; `all` explicitly allows unrestricted access.

See [thread sandbox bindings](./running-and-streaming.md#sandbox-bindings) for usage and availability.
