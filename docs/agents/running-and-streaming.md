...
Parameters must match the [factory configuration](./defining-agents.md#sandbox-configuration)
and cannot be changed after creation. Do not put secrets in them.

The first run prepares the environment; later runs resume its files and uncommitted edits without
replaying setup. Each run saves before completion. Saved files remain subject to provider retention.

If state is unavailable or uncertain, further runs fail with `sandbox_execution_unavailable`.
The chat offers explicit recreation, or use `sixb.agent.threads.recreateSandbox(thread.id,
{ expectedGeneration: thread.workspaceState.generation })`. Recreation starts fresh without
recovering edits or deleting the previous environment. Workflows and subagents remain ephemeral.
