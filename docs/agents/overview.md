...
export const sixb = createSixb({
  models: {
    language: [vercelGateway("openai/gpt-5.5"), vercelGateway("anthropic/claude-sonnet-4.6")],
  },
  tools: [lookup],
})
```
...
When several models are configured, the main Agent sees the default, capabilities, and context
limits declared by their providers when available. Prices and runtime speed are not guessed;
