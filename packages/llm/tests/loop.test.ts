...
import type { LanguageModelStreamEvent, ModelCallEndEvent, ModelTool, ModelUiChunk } from "../src"
import { ModelStreamError, runModelLoop, StructuredOutputError } from "../src"
import { MockLanguageModel, streamFromArray } from "../src/testing"
...
describe("runModelLoop", () => {
  ...
  test("records completed-call usage before rejecting invalid durable provider data", async () => {
    ...
    ).rejects.toThrow("provider data must be a finite, acyclic JSON value")
    expect(calls).toEqual([
      {
