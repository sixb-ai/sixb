...
import type { LanguageModelRequest, LanguageModelStreamEvent } from "@sixb/llm"
import { ModelProviderError } from "@sixb/llm"
import { createOpenResponsesProvider, vercelGateway } from "../src"
...
describe("OpenResponses provider", () => {
  test("maps requests and normalizes fragmented text/usage streams", async () => {
    ...
    const model = provider.model("creator/model", {
      request: { provider_options: { gateway: { order: ["one", "two"] } } },
    ...
    expect(capturedBody).toMatchObject({
      ...
      provider_options: { gateway: { order: ["one", "two"] } },
  ...
  test("streams tool arguments and preserves exact provider replay items", async () => {
    ...
    const model = provider.model("tool-model")
    const result = await model.stream(
      request({
        messages: [
          {
  ...
  test("sanitizes provider HTTP failures and validates convenience defaults", async () => {
    const gateway = vercelGateway({ apiKey: "key" })
    expect(gateway.id).toBe("vercel-gateway")

    expect(() => gateway.model("model", { request: { generatedAt: new Date() } as never })).toThrow(
      "model request options"
    )

    const model = createOpenResponsesProvider({
      id: "broken",
      baseUrl: "https://models.example/v1",
      fetch: async () =>
