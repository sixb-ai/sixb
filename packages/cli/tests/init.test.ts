...
describe("sixb init", () => {
  ...
  test("scaffolds the updated basic template with a root app folder", async () => {
    ...
    await stat(join(targetDir, "app", "public", "sixb-wordmark.svg"))
    await stat(join(targetDir, "connectors", "celestrak.ts"))
    await stat(join(targetDir, "datasets", "satellite-orbit.ts"))
    await stat(join(targetDir, "ontology", "satellite.ts"))
    await stat(join(targetDir, "projections", "satellite.ts"))
    await stat(join(targetDir, "syncs", "satellite-orbit.ts"))
    ...
    expect(packageJson.dependencies["@sixb/client"]).toBe("^0.1.9")
    expect(packageJson.dependencies["@sixb/ducklake"]).toBe("^0.1.5")
