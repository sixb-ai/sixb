  ...
  return applyAnthropicPricingModifiers(
    {
      ...
      cacheWriteInput: scale(rule.input, 5n, 4n),
    },
    modelId,
    request
  )
...
export function applyAnthropicPricingModifiers(
  ...
  const cacheControl = object(request?.cache_control)
  const cacheWriteMultiplier: readonly [bigint, bigint] =
    cacheControl?.ttl === "1h" ? [2n, 1n] : [5n, 4n]
  ...
  return {
    ...
    cacheWriteInput: scale(input, ...cacheWriteMultiplier),
  }
}
