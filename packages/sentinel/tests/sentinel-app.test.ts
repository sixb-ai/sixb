...
describe("createSentinelApp", () => {
  ...
  test("serves disabled auth state in the Sentinel shell", async () => {
    const port = await getFreePort()
    const sentinel = createSentinelApp({
      apiBaseUrl: "http://api.localhost",
      authEnabled: false,
    ...
    try {
      ...
      expect(await response.text()).toContain('"auth":{"audience":"sentinel","enabled":false}')
    } finally {
      await server.stop()
    }
  })
  ...
  test("does not serve Pario API-owned routes from the Sentinel origin", async () => {
    ...
    try {
      const baseUrl = `http://127.0.0.1:${port}`
      for (const path of ["/api/workflows", "/auth/sign-in", "/ws/events", "/docs"]) {
...
function extractAssetPath(html: string, kind: "script" | "stylesheet"): string {
  ...
  if (!match?.[1]) {
    throw new Error(`Could not find Sentinel ${kind} asset path in shell HTML.`)
  }
  return match[1]
}
