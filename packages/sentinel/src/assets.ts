...
export async function loadBuiltInUiBundle(
  ...
  try {
    ...
    throw new Error(
      `[ParioSentinel] Built Sentinel UI assets are missing in ${outdir}. Run \`pario build\` before serving Sentinel in production.`
    )
  }
...
function findBuiltAsset(files: readonly string[], entryName: string, extension: string): string {
  ...
  if (matches.length !== 1) {
    throw new Error(
      `[ParioSentinel] Expected one ${entryName}.${extension} bundle in the built Sentinel UI output, found ${matches.length}. Run \`pario build\`.`
    )
  }
