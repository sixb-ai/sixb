...
export async function buildBuiltInUiBundle(
  ...
  if (exitCode !== 0) {
    ...
    throw new Error(`[SixbSentinel] Failed to build built-in UI bundle: ${stderr.trim()}`)
  }
...
export async function loadBuiltInUiBundle(
  ...
  try {
    ...
    throw new Error(
      `[SixbSentinel] Built Sentinel UI assets are missing in ${outdir}. Run \`sixb build\` before serving Sentinel in production.`
    )
  }
...
function findBuiltAsset(files: readonly string[], entryName: string, extension: string): string {
  ...
  if (matches.length !== 1) {
    throw new Error(
      `[SixbSentinel] Expected one ${entryName}.${extension} bundle in the built Sentinel UI output, found ${matches.length}. Run \`sixb build\`.`
    )
  }
