...
export class LogSubscriptionHub {
  ...
  constructor(private readonly host: { readonly logs: LogsRuntime }) {}
  ...
  private async ensureStarted(): Promise<void> {
    ...
    if (!this.startPromise) {
      this.startPromise = this.host.logs
  ...
  private async catchUp(key: object, state: ClientState): Promise<void> {
    ...
    try {
      let hasMore = true
      while (hasMore) {
        const page = await this.host.logs.read({
          afterCursor,
      ...
      if (error instanceof BrokerCursorExpiredError) {
        try {
          const latest = await this.host.logs.tail({ limit: 1 })
          this.send(state, {
            type: "reset",
