...
const ReportItem = defineObjectType({
  id: "shared-report-item",
  name: "Report item",
...
const Report = defineObjectType({
  ...
  links: [link("items", ReportItem, { properties: [prop("position", "integer")] })],
})

const PublishedReport = defineShare("published-report", {
  target: Report,
  grants: ({ target }) => [can.view(target).withLinks([Report.l.items])],
})
...
describe("shared-access request boundary", () => {
  // Guard proof: removing sharedSession from queryObjectLinks makes the permitted query return 403.
  test("queries selected physical links and edge properties through the normal API", async () => {
    const fixture = await createFixture()
    const trusted = createTestSixb(fixture.host)
    for (const index of [1, 2]) {
      await trusted.objects(ReportItem).upsert({
        properties: { id: `item-${index}`, name: `Item ${index}` },
      })
      await trusted.objects.upsertLink(Report.id, `report-${index}`, "items", {
        targetTypeId: ReportItem.id,
        targetId: `item-${index}`,
    ...
    const { csrfToken } = await sharedSessionBody(exchanged)
    const query = (primaryId: string) =>
      fixture.app.fetch(
        new Request("http://api.localhost/api/objects/query/links", {
          method: "POST",
          headers: {
            cookie: cookies,
            "content-type": "application/json",
            "x-sixb-csrf": csrfToken,
            [SHARED_ACCESS_GRANT_HEADER_NAME]: invitation.grant.id,
          },
          body: JSON.stringify({
            query: { kind: "refs", refs: [{ objectTypeId: Report.id, primaryId }] },
            linkId: "items",
            direction: "outgoing",
            includeObjects: true,
          }),
    ...
    const selected = await query("report-1")
    expect(selected.status).toBe(200)
    const body = (await selected.json()) as {
      links: Array<{ properties: unknown }>
      objects: unknown[]
    }
    expect(body.links).toHaveLength(1)
    expect(body.links[0]?.properties).toEqual({ position: 1 })
    const sibling = await query("report-2")
    expect(sibling.status).toBe(200)
    expect(await sibling.json()).toMatchObject({ links: [], objects: [] })
  })
  ...
  test("never caches shared file GET or HEAD responses", async () => {
    ...
    for (const method of ["GET", "HEAD"] as const) {
      ...
      // Elysia then preserves the file helper's private revalidation cache header and this fails.
      ...
      expect(principal.headers.get("cache-control"), method).toBe("private, no-cache")
