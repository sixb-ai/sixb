...
export function FileRefAttachment({
  ...
  return (
    <Attachment size="sm" className="w-full max-w-[28rem] border-border/70 bg-background">
      ...
      {/*
        Content access is delegated to native browser navigation for this slice.
        View opens the inline disposition in a new tab; Download hits the
        attachment disposition, which the server forces via Content-Disposition,
        so the browser saves it in place (no new tab). Both URLs are grant-checked
        by the object file-content route. A fetch + object-URL flow with revoke
        (and a friendlier error surface than the raw server JSON) is a follow-up.
      */}
