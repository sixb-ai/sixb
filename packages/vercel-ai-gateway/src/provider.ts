...
/** Shared zero-configuration gateway, mirroring the callable provider DX of the AI SDK. */
...
class VercelGatewayLanguageModel implements LanguageModel {
  ...
  async stream(request: LanguageModelRequest) {
    const response = await (this.transport.fetch ?? fetch)(`${this.transport.baseUrl}/responses`, {
      method: "POST",
