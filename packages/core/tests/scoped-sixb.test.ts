...
describe("bound Sixb operational access", () => {
  ...
  test("agent run preflight rejects an exhausted project before creating a thread", async () => {
    ...
    await host.storage.auth!.sessions.create({
      id: "quota-test-session",
      projectId: host.id,
      userId: principal.id,
      strategyId: "test",
      audience: "atlas",
      tokenHash: "quota-test",
      createdAt: new Date(),
      expiresAt: new Date(Date.now() + 60_000),
    })
    const runner = bindRequestExecution(host, {
      ...
      authorization: {
        type: "principal",
        context: { ...contextFor(host, ["operations"]), sessionId: "quota-test-session" },
        credential: { type: "session", id: "quota-test-session" },
      },
    })
    await expect(
      runner.agent.runs.request({
