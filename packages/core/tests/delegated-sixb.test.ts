...
describe("delegated Sixb runtime", () => {
  test("enforces exact reads through the canonical Sixb APIs", async () => {
    ...
    // Guard proof: restoring raw ontology subtype expansion in list-service.ts rejects this read.
    ...
    expect(
      (
        await shared.objects.queryLinks({
          query: linkQuery,
          direction: "outgoing",
    ...
    // Guard proof: removing selected-read admission from AuthorizedObjectReader makes this return
  ...
  test("enforces the selected traversal budget on object lists", async () => {
    const host = createRuntime()
    const sixb = createTestSixb(host)
    await sixb.objects(Proposal).upsert({
