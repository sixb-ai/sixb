...
import { sandboxConfig } from "../src/sandboxes"
import { createTestRuntimeDeps } from "./test-runtime-deps"

// Regression proof: erase the sandbox type parameter from SixbHost.withScope or createSixb.
function checkHost(scope: ExecutionScope) {
  ...
  sixb.agent.threads.create({ sandbox: { clientId: "acme" } })
  // @ts-expect-error clientId is required
  sixb.agent.threads.create({ sandbox: {} })
  // @ts-expect-error clientId must be a string
  sixb.agent.threads.create({ sandbox: { clientId: 42 } })
  // @ts-expect-error unknown parameters are rejected
  sixb.agent.threads.create({ sandbox: { clientId: "acme", other: true } })
}
...
async function checkDiscoveredHost(options: CreateSixbOptions, scope: ExecutionScope) {
  ...
  // @ts-expect-error createSixb must preserve the sandbox parameter schema too
