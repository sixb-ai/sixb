...
import { type AuthorizationContext, emptyGrantIndex, isRuntimeAllowed } from "../src/authorization"
...
describe("runtime authorization capabilities", () => {
  test("snapshots principal authority and exposes only a defensive durable reference", () => {
    ...
    expect(
      isRuntimeAllowed(
        { projectId: "project-1", runtimeAuthorization: authorization },
        { kind: "agent.run", agentId: "injected-agent" }
      )
    ).toBe(false)
    expect(
      isRuntimeAllowed(
        { projectId: "project-1", runtimeAuthorization: authorization },
        { kind: "agent.run", agentId: "original-agent" }
      )
    ).toBe(true)
