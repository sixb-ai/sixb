...
import { type RuntimeAccessPlan, snapshotRuntimeAccessPlan } from "../authorization/access-plan"
...
import { type DelegatedExecutionLimits, snapshotDelegatedExecutionLimits } from "./limits"
import {
...
export type ResolvedRuntimeAuthorization =
  ...
  | {
      ...
      readonly ref: RuntimeDelegationRef
    }
  | { readonly type: "denied" }

/** Process-local delegation provenance. Durable shared attribution lands with shared sessions. */
export interface RuntimeDelegationRef {
  readonly type: "delegated"
...
/** Register narrowly delegated authority without fabricating a principal identity. */
export function createDelegatedRuntimeAuthorization(input: {
  ...
  assertNonEmpty(input.delegation.kind, "Delegation kind")
  assertNonEmpty(input.delegation.id, "Delegation id")
  if (input.delegation.sessionId !== undefined) {
    assertNonEmpty(input.delegation.sessionId, "Delegation session id")
  }
  return register(input.execution, {
    type: "delegated",
    projectId: input.execution.projectId,
    access: snapshotRuntimeAccessPlan(input.access),
    limits: snapshotDelegatedExecutionLimits(input.limits),
    ref: Object.freeze({
      type: "delegated",
      kind: input.delegation.kind,
      id: input.delegation.id,
      ...(input.delegation.sessionId === undefined
        ? {}
        : { sessionId: input.delegation.sessionId }),
    }),
  })
}
 ...
 *
 * Bound runtime façades carry no second execution value to recombine. Any lower-level boundary
...
/** Fail before a request can observe or mutate durable state it is not yet possible to attribute. */
export function assertRuntimeAuthorizationCanCrossDurableBoundary(
  authorization: RuntimeAuthorization
): void {
  void resolveDurableRuntimeAuthorization(authorization)
}
