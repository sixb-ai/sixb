...
import { applyEditOperation } from "./edit-operations"
import {
  resolveLink,
  resolveObject,
...
import { normalizeOntologyEditCommit } from "./normalize"
import { linkRefKey, linkRefSortKey, objectRefKey, objectRefSortKey } from "./refs"
import type {
  EditCommitResult,
  EffectiveLinkChange,
...
import { drainStagedEvents, drainStagedWork, stageWorkBounded } from "./work-executor"
import {
...
export async function commitEdits(
  ...
  if (replay) {
    if (input.source.kind === "action") {
      await attachRunReplayTransaction(context, "action", input.source.runId, identity.commitId)
    }
    return replay
  }
