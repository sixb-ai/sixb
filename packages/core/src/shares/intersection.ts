...
import { type ShareAccessPlan, snapshotShareAccessPlan } from "./access-plan"
...
export function intersectShareAccessPlans(
  issued: ShareAccessPlan,
  current: ShareAccessPlan
): ShareAccessPlan {
  const issuedSnapshot = snapshotShareAccessPlan(issued)
  const currentSnapshot = snapshotShareAccessPlan(current)
  const grants: ShareAccessPlan["grants"][number][] = []
...
function viewRoots(plan: ShareAccessPlan): readonly ObjectReadRoot[] {
