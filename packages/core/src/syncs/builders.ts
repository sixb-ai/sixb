 ...
 * V1 supports `snapshot` and `append` modes with optional schedule attachment.
 */
export function defineSync<TId extends string>(
  ...
  const builder: SyncBuilder<TId> = {
    ...
    from<TConnector extends SyncDefinition["connector"]>(
      connector: TConnector
    ): SyncReadBuilder<TId, TConnector> {
      return {
        read(handler): SyncTargetBuilder<TId, TConnector> {
          return {
