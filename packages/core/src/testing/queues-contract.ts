...
export function runQueueContractSuite(label: string, options: QueueContractSuiteOptions): void {
  ...
  describe(label, () => {
    describe("enqueue", () => {
      ...
      test("coalesces repeated pending refresh jobs", async () => {
        await withQueues(async (queues) => {
              ...
              jobs: [
                {
                  ...
                  payload: { pipelineId: "customers", runId },
                  coalesce: { key: "pipeline:customers" },
                },
              ],
            })
          }
          ...
          expect(claimed[0]?.job.payload.pipelineId).toBe("customers")
        })
      })

      test("keeps only the latest successor while a coalesced job is active", async () => {
        await withQueues(async (queues) => {
          await queues.pipelines.enqueue({
            projectId: "project-a",
            jobs: [
              {
                type: "pipeline.run.requested",
                payload: { pipelineId: "customers", runId: "run-active" },
                coalesce: { key: "pipeline:customers" },
              },
            ],
          })
          const [active] = await queues.pipelines.claim({
            projectId: "project-a",
            workerId: "worker-1",
          })
          expect(active).toBeDefined()

          for (const runId of ["run-stale", "run-latest"]) {
            await queues.pipelines.enqueue({
              projectId: "project-a",
              jobs: [
                {
          ...
          await queues.pipelines.complete({
            projectId: "project-a",
            jobId: active!.job.id,
            leaseId: active!.leaseId,
          })
          const successor = await queues.pipelines.claim({
            projectId: "project-a",
            workerId: "worker-1",
            limit: 2,
          })
          ...
          expect(successor[0]?.job.payload.runId).toBe("run-latest")
        })
      })
