    ...
    test("rejects invalid records and list windows consistently", async () => {
      await withStorage(async (storage) => {
        ...
        const invalid: CreateShareGrantInput[] = [
          ...
          grantInput({ destinationPath: "/shared" }),
          grantInput({ destinationPath: "/shared/report-1" }),
          grantInput({ destinationPath: "/shared%2Freport-1" }),
          grantInput({ destinationPath: "/shared%5Creport-1" }),
          grantInput({ destinationPath: "/%73hared/report-1" }),
