# @pario/workflow-worker

Sequential workflow runtime for Pario.
...
normal Pario action path, waits for terminal action events through `requestActionAndWait(...)`, and emits one final `workflow.run.finished` event.
