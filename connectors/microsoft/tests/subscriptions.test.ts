...
describe("Graph subscriptions", () => {
  ...
  // Countercheck: restoring a seven-day cap in renew rejects this valid SharePoint request.
  test("renew accepts SharePoint lifetimes longer than seven days through the shared update path", async () => {
    const requests = mockFetch(() => json({ id: "sub" }))
    const client = await connect()
    const expirationDateTime = new Date(Date.now() + 20 * 86_400_000).toISOString()
    await client.subscriptions.renew("sub", expirationDateTime)
    expect(requests[0]?.method).toBe("PATCH")
    expect(JSON.parse(String(requests[0]?.init.body))).toEqual({ expirationDateTime })
    await expect(client.subscriptions.renew("sub", "invalid")).rejects.toBeInstanceOf(
      MicrosoftConfigurationError
    )
