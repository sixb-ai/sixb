...
class VercelGatewayLanguageModel implements LanguageModel {
  ...
  async stream(request: LanguageModelRequest) {
    ...
    const requestId = response.headers.get("x-request-id") ?? response.headers.get("request-id")
    return {
      events: responsesEvents(response.body, request.signal, {
        providerId: this.providerId,
        modelId: this.modelId,
        requestId: requestId ?? undefined,
        errorPrefix: "[SixbVercelGateway]",
        providerIds: gatewayProviderIds,
  ...
  private async prepareRequest(request: LanguageModelRequest): Promise<{
    ...
    return {
      body: {
        ...extra,
        model: this.modelId,
        input: responsesInput(request.messages, this.providerId),
        stream: true,
...
function normalizeUsage(raw: JsonObject | undefined): ModelUsage {
  ...
  const { inputTokens, outputTokens, reasoningOutputTokens: reasoning } = usage
  const cachedRaw = usage.cacheReadInputTokens
