...
import { SmolvmSandboxFactory, type SmolvmSandboxFactoryOptions } from "../src"
...
async function checkConfiguration(
  ...
  providerOptions: SmolvmSandboxFactoryOptions,
  scope: ExecutionScope
) {
  const factory: SandboxFactory = new SmolvmSandboxFactory(providerOptions)
  void factory

  const params = { clientId: param("string"), branch: optional(param("string")) }
  const configured: SmolvmSandboxFactoryOptions<typeof params> = {
    ...providerOptions,
    params,
    resolve: ({ params }) => {
  ...
  const sandboxes = new SmolvmSandboxFactory(configured)
  const inferred: SandboxFactory<typeof params> = new SmolvmSandboxFactory({
    ...providerOptions,
    params,
    resolve: ({ params }) => {
  ...
  const widened: SmolvmSandboxFactoryOptions = configured
  void widened
