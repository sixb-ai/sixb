...
import { AppleContainerSandboxFactory, type AppleContainerSandboxFactoryOptions } from "../src"
...
async function checkConfiguration(
  ...
  providerOptions: AppleContainerSandboxFactoryOptions,
  scope: ExecutionScope
) {
  const factory: SandboxFactory = new AppleContainerSandboxFactory(providerOptions)
  void factory

  const params = { clientId: param("string"), branch: optional(param("string")) }
  const configured: AppleContainerSandboxFactoryOptions<typeof params> = {
    ...providerOptions,
    params,
    resolve: ({ params }) => {
  ...
  const sandboxes = new AppleContainerSandboxFactory(configured)
  const inferred: SandboxFactory<typeof params> = new AppleContainerSandboxFactory({
    ...providerOptions,
    params,
    resolve: ({ params }) => {
  ...
  const widened: AppleContainerSandboxFactoryOptions = configured
  void widened
