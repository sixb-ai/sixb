...
async function checkConfiguration(
  ...
  providerOptions: AzureSandboxFactoryOptions,
  scope: ExecutionScope
) {
  const factory: SandboxFactory = new AzureSandboxFactory(providerOptions)
  void factory
  ...
  const sandboxes = new AzureSandboxFactory(configured)
  const inferred: SandboxFactory<typeof params> = new AzureSandboxFactory({
    ...providerOptions,
    params,
    resolve: ({ params }) => {
