...
provider catalog. If no limit is available, the worker uses a 128,000-token fallback with a warning.
Configure `loop.context.windowTokens` or supply a model definition to override that assumption.
