...
export async function listAiModelCallGroups(
  ...
  const count = connection.db.query(COUNT_SQL).get(...parameters) as { total: string }
  const rows = connection.db
    .query(PAGE_SQL)
    .all(...parameters, query.limit, query.offset) as AiModelCallGroupRow[]
...
WITH filtered AS (
  SELECT COALESCE(root_agent.execution_id, usage.execution_id) AS root_execution_id,
    ...
    root_agent.agent_id AS root_agent_id,
    ...
    workflow_agent.agent_id AS workflow_agent_id,
  ...
  WHERE usage.project_id = ?1
    AND usage.occurred_at >= ?2
    AND usage.occurred_at < ?3
    AND (?4 IS NULL OR usage.provider_id = ?4)
    AND (?5 IS NULL OR usage.requested_model_id = ?5)
    AND (?6 IS NULL OR COALESCE(cost.status, 'unvalued') = ?6)
), roots AS (
...
const PAGE_SQL = `${FILTERED_SQL}, page AS (
  ...
  ORDER BY last_call_at DESC, root_execution_id ASC LIMIT ?7 OFFSET ?8
)
SELECT filtered.root_execution_id, execution_id, provider_id, requested_model_id,
  ...
  MAX(root_agent_id) AS root_agent_id,
  ...
  MAX(workflow_agent_id) AS workflow_agent_id,
