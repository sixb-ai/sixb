ALTER TABLE executions
  ADD COLUMN authority_shared_grant_id TEXT,
  ADD COLUMN authority_shared_session_id TEXT;
...
BEGIN
  ...
  LOOP
    EXECUTE format('ALTER TABLE executions DROP CONSTRAINT %I', check_name);
  END LOOP;
END $$;

ALTER TABLE executions
  ADD CONSTRAINT executions_authority_kind_check CHECK (
    authority_kind IN ('principal', 'trustedPrimitive', 'sharedAccess', 'kernel', 'disabled')
  ),
  ADD CONSTRAINT executions_authority_shape_check CHECK (
    (
      ...
      AND authority_shared_grant_id IS NULL
      AND authority_shared_session_id IS NULL
    )
    OR (
    ...
    OR (
      ...
      AND authority_primitive_id IS NULL
      AND authority_kernel_operation IS NULL
      AND authority_shared_grant_id IS NOT NULL
      AND length(trim(authority_shared_grant_id)) > 0
      AND authority_shared_session_id IS NOT NULL
      AND length(trim(authority_shared_session_id)) > 0
    )
    OR (
  ...
  ADD CONSTRAINT executions_executor_authority_check CHECK (
    (
      ...
      AND (
        ...
        OR (
          authority_kind IN ('sharedAccess', 'disabled')
