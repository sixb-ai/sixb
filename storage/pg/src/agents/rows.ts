...
export interface AgentThreadRow {
  ...
  title: string | null
  sandbox_params: JsonValue | string | null
  sandbox_state: AgentThreadSandboxState | string | null
...
export function rowToThreadRecord(row: AgentThreadRow): AgentThreadRecord {
  return {
    ...
    ...(row.sandbox_state == null
      ...
      : {
          sandboxState:
            typeof row.sandbox_state === "string"
              ? JSON.parse(row.sandbox_state)
              : row.sandbox_state,
        }),
    ...(row.sandbox_params == null
      ? {}
      : {
          sandboxParams: snapshotAgentThreadSandboxParams(
            typeof row.sandbox_params === "string"
              ? JSON.parse(row.sandbox_params)
              : row.sandbox_params
          ),
        }),
    status: row.status,
